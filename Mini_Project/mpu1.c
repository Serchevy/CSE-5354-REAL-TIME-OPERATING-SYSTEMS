...
int8_t find_SR(void *ptr) {
    ...
    if(add <= 0x1000) {                          // Region 0
        region = 0;
        size = 512;
        region_offset = 0x0000;
    }
    else if(add > 0x1000 && add <= 0x3000) {    // Region 1
        region = 1;
        size = 1024;
        region_offset = 0x1000;
    }
    else if(add > 0x3000 && add <= 0x4000) {    // Region 2
        region = 2;
        size = 512;
        region_offset = 0x3000;
    }
    else if(add > 0x4000 && add <= 0x6000) {    // Region 3
        region = 3;
        size = 1024;
        region_offset = 0x4000;
    }
    else if(add > 0x6000 && add <= 0x7000) {    // Region 4
        region = 4;
        size = 512;
        region_offset = 0x6000;
    }
    ...
    subReg = region*8 + subReg_offset - 1;
...
void *malloc_from_heap(uint32_t size_in_bytes) {
    ...
    if(size_in_bytes <= 0x200) {                                        // size <= 512 check sub-regs R0, R2, R4
        for(i=0; i<33; i+=16) {
            for(j=i; j<8+i; j++) {
                if(!(subRegInUse & (1 << j))) {
                    if(0x200 < best_fit) {
    ...
    else if(size_in_bytes > 0x200 && size_in_bytes <= 0x400 ) {         // size <= 1024 check sub-regs R1, R3,
        for(i=8; i<25; i+=16) {
            for(j=i; j<8+i; j++) {
                if(!(subRegInUse & (1 << j))) {
                    if(0x400 < best_fit) {
        ...
        for(i=7; i<33; i+=8) {
            if(!(subRegInUse & (1 << i)) && !(subRegInUse & (1 << i+1))) {
                if(0x600 < best_fit) {
                    best_fit = 0x600;
                    best_idx = i+1;
                }
    ...
    else if(size_in_bytes > 0x600 && size_in_bytes <= 0x1000) {         // size <= 4096 -> Regions R0, R2, R4
    ...
    else if(size_in_bytes > 0x1000 && size_in_bytes <= 0x2000) {        // size <= 8192 -> Regions R1, R3
    ...
    for(i=0; i<subregs_to_use; i++) subRegInUse |= 1 << (best_idx - i);     // Update mask

    for(j=0; j<12; j++) {                                                   // Find task slot not in use
        if(!HCB_table[j].inUse) {                                           // Fill in meta-data
            HCB_table[j].inUse = 1;
            HCB_table[j].ptr = address(best_idx);
            HCB_table[j].size = size_in_bytes;
            HCB_table[j].PID = PID++;
...
void free_to_heap(void *ptr) {
    ...
    for(i=0; i<12; i++) {
        if(HCB_table[i].ptr == ptr) {
            ...
            if((start >= 8 && start <= 15) || (start >=24 && start <=31)) subreg_size = 1024;
            else subreg_size = 512;
            subregs_used = (HCB_table[i].size + subreg_size - 1)/subreg_size;       // Determine num of sub-regions used

            for(i=0; i<subregs_used; i++) subRegInUse &= ~(1 << (start - i));       // Update mask
