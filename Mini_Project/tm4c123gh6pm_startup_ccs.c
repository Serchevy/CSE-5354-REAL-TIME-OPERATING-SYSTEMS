...
#include "asp.h"
#include "uart0.h"  // need it for now
...
#pragma DATA_SECTION(heapArray, ".heap")
char heapArray[0x7000];
...
static void BusFaultISR() {
    display("Bus Fault in Process ", pid, 0);
    while(true);
}

static void UsageFaultISR() {
    display("Usage Fault in Process ", pid, 0);
    while(true);
}

static void HardFaultISR() {
    display("Hard Fault in Process ", pid, 0);
    putsUart0("------------------\n");
    ...
    display("PSP:    0x", PSP, 1);
    display("MSP:    0x", MSP, 1);
    display("mflags: 0x", mfault, 1);

    // print the offending instruction.
    uint16_t instr = *( (uint32_t *)(*(PSP+6)));
//    uint32_t instr = (temp << 16) | (temp >> 16);

    putsUart0("------------------\n");
    display("ofsIns: 0x", instr, 1);
    putsUart0("------------------\n");
...
static void MPUFaultISR() {
    display("MPU Fault in Process ", pid, 0);
...
static void SysTickISR() {
    display("SysTick in Process ", pid, 0);
